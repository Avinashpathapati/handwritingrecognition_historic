...
def path_search_dp(im,st, end):
  ...
  cost_path_arr = np.full((im.shape[0], (end-st)+1), sys.maxint)
...
def extract_char_save_fold(short_path_arr,im,st,end_seg,line_num,word_num,x_min,x_max,path_present,scrol_name):
  ...
  if path_present:
    ...
    cv.imwrite(os.path.join(save_path,'char_'+str(st)+str(st+end)+str(x_min)+str(x_max)+'.png'), char_im)
    cv.imwrite(os.path.join(save_path,'char_'+str(st_2_seg)+str(end_seg-1)+str(x_min)+str(x_max)+'.png'), char_im2)
  else:
    char_im = im[0:x_max,st:end_seg]
    cv.imwrite(os.path.join(save_path,'char_'+str(st)+str(end_seg)+str(x_min)+str(x_max)+'.png'), char_im)
    ...
    for i in range(1,len(char_seg_col),2):
      ...
      if not zero_flag:
        ...
        if (char_seg_col[i-1]+short_path_arr[len(short_path_arr)/2]) - char_seg_col[i-1] > 30:
          ...
          extract_char_save_fold(short_path_arr,im,char_seg_col[i-1],char_seg_col[i]+1,im_ct,cur_word_num+1,0,im.shape[0],True,scrol_name)
        else:
          extract_char_save_fold(short_path_arr,im,char_seg_col[i-1],char_seg_col[i]+1,im_ct,cur_word_num+1,0,im.shape[0],False,scrol_name)
